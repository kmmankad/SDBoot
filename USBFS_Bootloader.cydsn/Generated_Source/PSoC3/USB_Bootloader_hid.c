...
extern uint8 CYCODE USB_Bootloader_HIDREPORT_DESCRIPTORS[];
...
{
    uint8 status = USB_Bootloader_IDLE_TIMER_INDEFINITE;
    ...
    if(USB_Bootloader_hidIdleRate[interface] != 0u)
    {
        if(USB_Bootloader_hidIdleTimer[interface] > 0u)
        {
            ...
            status = USB_Bootloader_IDLE_TIMER_RUNNING;
        }
        else
        {
            USB_Bootloader_hidIdleTimer[interface] = USB_Bootloader_hidIdleRate[interface];
            status = USB_Bootloader_IDLE_TIMER_EXPIRED;
        }
    }
...
*
*******************************************************************************/
uint8 USB_Bootloader_DispatchHIDClassRqst() 
{
...
*
*******************************************************************************/
void USB_Bootloader_FindHidClassDecriptor() 
{
    ...
    pTmp = (T_USB_Bootloader_LUT *) pTmp->p_list;
    ...
    USB_Bootloader_currentTD.count = pDescr[0u];
...
*
*******************************************************************************/
void USB_Bootloader_FindReportDescriptor() 
{
    T_USB_Bootloader_LUT *pTmp;
    volatile uint8 *pDescr;
    uint8 interfaceN;
    ...
    pTmp += 3;
    pDescr = (volatile uint8 *)pTmp->p_list;
    USB_Bootloader_currentTD.count =  (((uint16)pDescr[1] << 8u) | pDescr[0u]);
...
*
*******************************************************************************/
void USB_Bootloader_FindReport() 
{
    T_USB_Bootloader_LUT *pTmp;
    ...
    if(interfaceN < USB_Bootloader_MAX_INTERFACES_NUMBER)
    {
           ...
           (reportType <= USB_Bootloader_HID_GET_REPORT_FEATURE))
        {
            ...
            if(pTmp->c > reportType)
            {
                ...
                USB_Bootloader_currentTD.pData = pTD->pData;   /* Bufer pointer */
                USB_Bootloader_currentTD.count = pTD->count;   /* Bufer Size */
